...
describe("User Analysis Verification", () => {
  it("should verify the user's analysis about retain(0) transform", () => {
    ...
    console.log("A的操作:", JSON.stringify(opA.ops));
    console.log("B的操作:", JSON.stringify(opB.ops));

    // 1. A 先到达服务端，直接应用
    const afterA = base.compose(opA);
    console.log("A到达后服务端内容:", JSON.stringify(afterA.ops));

    // 2. B 后到达服务端，需要transform
    const transformedB = OTEngine.transform(opA, opB);
    console.log("服务端transform后的B:", JSON.stringify(transformedB.ops));

    // 3. 服务端最终内容
    const finalServer = afterA.compose(transformedB);
    console.log("服务端最终内容:", JSON.stringify(finalServer.ops));

    // 4. 客户端B收到A的操作，需要transform
    const transformedA = OTEngine.transform(opB, opA);
    console.log("B transform A后的结果:", JSON.stringify(transformedA.ops));
